...
| AI agent autodetect order or `AICAP_*_MODEL` defaults | Same change in **all three**: `dot_config/zsh/tools/04_ai_capture.zsh` (`_aiagent_autodetect`, defaults), `dot_config/tmux/executable_tmux-session-summary.py` (`detect_agent`, `AGENT_CONFIG` dict order, defaults), `scripts/aiblock.py` (`AGENT_CONFIG` dict order, defaults). Each agent's `-m`/`--model` flag MUST be conditional on the env var being non-empty so users can fall back to vendor defaults when a pinned model ID is retired. Current order: `opencode → claude → codex → cursor-agent` (opencode-first per 2026-05-12 benchmark) | — |
