...
| `bin/executable_fleet` / `scripts/fleet/` / `scripts/fleet_apply.py` / `justfile` `fleet` + `fleet-*` recipes / `dot_config/fleet/` / sudo helper consumption / `dot_config/tmux/executable_tmux-session-summary.py` `--json` schema | [docs/this_repo/fleet-apply.md](docs/this_repo/fleet-apply.md) | The `--json` schema emitted by `tmux-session-summary.py` and the raw-fallback parser in `scripts/fleet/tmux.py:_parse_raw` MUST stay in sync — adding a field to `Session`/`Window` dataclass means updating both. |
