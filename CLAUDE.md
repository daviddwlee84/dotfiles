...
| New upstream CLI that ships `--completion <shell>` | Add a `regen <tool> "<zsh-args>" "<bash-args>"` row to `scripts/generate_completions.sh`; verify with `just completions-refresh`. `run_after_50_generate_completions.sh.tmpl` then regenerates on every apply (binary-mtime check, ~21ms cached). | [docs/zsh/zsh-completions.md](docs/zsh/zsh-completions.md) § A — current 16-tool inventory + what's deliberately handled elsewhere |
