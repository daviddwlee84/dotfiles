...
| Runtime specs in `dot_config/mise/config.toml.tmpl` `[tools]` | Every `mise install <tool>@<spec>` in `dot_ansible/**` must use the **same spec string** — mise keys `installs/<tool>/<spec>` by what you ask for, so `rust@latest` against a `rust = "stable"` config installs a working toolchain that mise still calls `(missing)` forever; `creates:` guards can't see a spec mismatch. Prefer bare `mise install --yes`. A **new runtime is two edits**: declare it here AND in `lazyvim_deps`' `Install mise global tools` — the only task that installs it, on all three OS families. | [pitfalls/mise-warn-missing-rust-stable-every-prompt.md](pitfalls/mise-warn-missing-rust-stable-every-prompt.md) |
