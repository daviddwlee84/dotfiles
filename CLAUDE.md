...
| `dot_dotfiles/bin/executable_appsrc` (single-file — detection + sizing + inline `AppsrcTUI`) / `dot_config/television/cable/appsrc.toml` + `appsrc-size.toml` / `dot_config/{zsh/tools,bash}/57_appsrc_completion.*` | Mirror single-letter mnemonics between the `appsrc tui` `BINDINGS` (`s`ize/`o`source/`n`ame/`y`copy/`r`eveal) and the tv channels' `Alt+<letter>` actions — change one → change both (same rule as the `mlf` row; tv = fast picker, `appsrc tui` = sortable dashboard). The `scan --tsv` column order (`name·source·path·kind·package[·size]`) is the pickers' wire format — reorder a column → fix both `.toml` `{split:\t:N}` indices. | All logic (detection/sizing/TUI) lives in the one launcher — NO `scripts/appsrc/` package; the TUI calls the same module functions. `textual` is PEP723-only, imported lazily in `do_tui` (never on `scan`/`which`/`size`) — no `python_uv_tools` entry, like `rich`. [`docs/tools/appsrc.md`](docs/tools/appsrc.md). |
