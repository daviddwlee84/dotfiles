...
| Console logging in any shell script | **Never re-hand-roll `info`/`warn`/`success`/`error` + a colour block** — `scripts/lib/log_shared.sh` is the SSOT for all 13 consumers. run-scripts `{{ include }}` it (they render to a temp path) and MUST set `LOG_STREAM=stdout` first, or fleet-apply's local-host `_classify_drift()` demotes their stderr from `drift` to `failed`; `scripts/*.sh` `source` it; files deployed to `$HOME` can't use it at all (`scripts/**` is chezmoi-ignored). Editing it rehashes every `run_onchange_after_*` that inlines it → full ansible + brew re-run on every host, so batch edits. New consumer → add it to the wiring-guard test in `tests/unit/log_shared.bats`. | [shell-logging](docs/this_repo/shell-logging.md) — API, `LOG_PREFIX`/`LOG_STREAM`/`NO_COLOR`, and the `set -e`/`set -u`/`IFS`/top-level-`return` traps the implementation avoids |
...
| New in-house CLI in `dot_dotfiles/bin/executable_*` (or new subcommand) | Add tab completion for **both** shells per the decision flow in [docs/zsh/zsh-completions.md](docs/zsh/zsh-completions.md) § F; hand-written = two files (`dot_config/zsh/tools/<NN>_<name>_completion.zsh` + `dot_config/bash/<NN>_<name>_completion.bash`) kept in sync. Add a row to that § F table, and to [docs/shells/aliases.md](docs/shells/aliases.md) if it ships an `<tool>-update-completion` helper. | § F also records the two existing strategies (tyro self-gen vs hand-written eager-load) and the dynamic-candidate pattern |
| Tool inventory / in-house CLIs / `tv` channels / keymaps **surfaced to agents** | `dot_agents/skills/chezmoi-dotfiles/SKILL.md.tmpl` — re-renders every `chezmoi apply`, so freshness is automatic; only edit it to gate a section on a **new prompt key** or name a stable new CLI. Keep it lean (pointers over enumerations). | [docs/tools/agent-skills.md](docs/tools/agent-skills.md) § First-party templated skill — incl. the `.chezmoiignore` single-`*` re-include trap |
...
For **non-interactive** injection (`scripts/fleet/apply.py` over SSH), `sudo_session_init` adopts `CHEZMOI_SUDO_PASSWORD_FILE` (0600) instead of prompting on `/dev/tty` — **never** read that variable directly, always go through the helper. Full API + state + cleanup model: [docs/this_repo/sudo-session.md](docs/this_repo/sudo-session.md).
