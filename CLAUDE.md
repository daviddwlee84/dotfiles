...
| Yazi Git status: `dot_config/yazi/{package.toml,init.lua,yazi.toml,plugins/git-guard.yazi/main.lua}` | The latest `git.yazi` requires Yazi 26.8.15. Setup MUST stay `pcall`-guarded, and both fetchers MUST run through `git-guard`: compatible hosts delegate to the plugin; older hosts use the built-in `noop` fetcher rather than accumulating stuck tasks. Keep both `id` (26.1 parser) and `group` (26.8 parser) in each rule. The guard is byte-identical to the Windows repo copy. | [yazi](docs/tools/yazi.md) |
...
| Runtime specs in `dot_config/mise/config.toml.tmpl` `[tools]` | Every `mise install <tool>@<spec>` in `dot_ansible/**` must use the **same spec string** — mise keys `installs/<tool>/<spec>` by what you ask for, so `rust@latest` against a `rust = "stable"` config installs a working toolchain that mise still calls `(missing)` forever; `creates:` guards can't see a spec mismatch. Prefer bare `mise install --yes`. **.NET** keeps `dotnet.isolated = true` and the same `10` spec in `dotnet_tools`; use `mise install`, never `mise use` against managed config, and verify SDK/global-tool startup through the selected runtime (directory or binary presence is insufficient). See [azure-cost runtime discovery](pitfalls/azure-cost-you-must-install-dotnet.md). A **new runtime is two edits**: declare it here AND in `lazyvim_deps`' `Install mise global tools` — the only task that installs it, on all three OS families. | [pitfalls/mise-warn-missing-rust-stable-every-prompt.md](pitfalls/mise-warn-missing-rust-stable-every-prompt.md) |
