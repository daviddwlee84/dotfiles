...
| New in-house CLI in `dot_dotfiles/bin/executable_*` (or new subcommand) | Add tab completion for **both** shells per the decision flow in [zsh-completions.md](docs/zsh/zsh-completions.md) § F; hand-written = two files (`dot_config/zsh/tools/<NN>_<name>_completion.zsh` + `dot_config/bash/<NN>_<name>_completion.bash`) kept in sync. Add a row to that § F table, and to [aliases.md](docs/shells/aliases.md) if it ships an `<tool>-update-completion` helper. | § F also records the two existing strategies + the dynamic-candidate pattern |
| Tool inventory / in-house CLIs / `tv` channels / keymaps **surfaced to agents** | `dot_agents/skills/chezmoi-dotfiles/SKILL.md.tmpl` — re-renders every `chezmoi apply`, so freshness is automatic; only edit it to gate a section on a **new prompt key** or name a stable new CLI. Keep it lean (pointers over enumerations). | [agent-skills.md](docs/tools/agent-skills.md) § First-party templated skill — incl. the `.chezmoiignore` single-`*` re-include trap |
