...
| zsh ZLE widgets | `dot_config/zsh/tools/{11_tools_picker,12_television,13_keys_picker,22_sesh,05_aisuggest}.zsh` | `Alt+T/R/P/G/E/A/I/S` (pickers), `Alt+/` (keys-picker — which-key-style cheatsheet, see [docs/shells/keybindings.md](docs/shells/keybindings.md)), `Alt+;` (aisuggest, configurable via `AISUGGEST_KEY`); rebound from `zvm_after_init` in `dot_zshrc.tmpl` to survive zsh-vi-mode's keybind wipe |
