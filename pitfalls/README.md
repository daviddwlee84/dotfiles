...
| [`copilot-proxy-start-hangs-at-resolving-dependencies`](copilot-proxy-start-hangs-at-resolving-dependencies.md) | `copilot-proxy: did not come up in time — check 'copilot-proxy logs'.`; `copilot-proxy logs` shows only `nohup: ignoring input` + `Resolving dependencies`; wedged `bun add @jeffreycao/copilot-api@… --no-summary` in `ps`; every retry stacks another zombie pair and none binds port 4141; `curl` to the npm registry works fine so it looks like a non-network fault | workaround documented (`copilot-proxy doctor` flags the stale installer; clear bun's cache lock + start with `ALL_PROXY`/`HTTPS_PROXY` stripped — bun stalls resolving through the socks proxy); permanent fix in `start` not yet wired up |
