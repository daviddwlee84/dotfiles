...
BUNDLES: dict[str, dict[str, object]] = {
    ...
    "minimal": {
        # Everything stays at prompt defaults (mostly off).
    },
    "custom": {
...
def build_chezmoi_argv(
    ...
    *,
    ...
    for key in ("email", "name"):
        if key in answers:
            argv += ["--promptString", f"{key}={answers[key]}"]
    # Choice prompt
    if "profile" in answers:
        argv += ["--promptChoice", f"profile={answers['profile']}"]
    ...
    for p in PROMPTS:
        ...
        argv += ["--promptBool", f"{p.key}={val_str}"]
    return argv
...
def doctor_scan(source_dir: Path) -> tuple[set[str], set[str], set[str]]:
    """Return (tmpl_keys, dockerfile_keys, script_keys)."""
    tmpl = (source_dir / ".chezmoi.toml.tmpl").read_text()
    tmpl_keys = set(re.findall(
        r'prompt(?:String|Bool|Choice)Once\s+\.\s+"(\w+)"', tmpl))

    dockerfile = (source_dir / "Dockerfile").read_text()
    # ARG CHEZMOI_INSTALL_FOO=... → camelCase back
    def _arg_to_camel(s: str) -> str:
        parts = s.lower().split("_")
        return parts[0] + "".join(p.title() for p in parts[1:])
    docker_args = re.findall(r"ARG\s+CHEZMOI_(\w+)\s*=", dockerfile)
    # REPO is not a prompt
    docker_keys = {_arg_to_camel(a) for a in docker_args if a.lower() != "repo"}

    script_keys = {p.key for p in PROMPTS}
    return tmpl_keys, docker_keys, script_keys


def doctor_report(source_dir: Path) -> int:
    tmpl, docker, script = doctor_scan(source_dir)
    all_keys = tmpl | docker | script
    ...
    if drift:
        ...
        return 1
    console.print("[green]All three surfaces agree.[/green]")
    return 0
...
class InitCmd:
    ...
    yes: Annotated[bool, tyro.conf.arg(help="Skip final confirmation")] = False
...
def run_init(cmd: InitCmd) -> int:
    ...
    if not pf.is_tty:
        console.print("[red]No TTY detected — cannot run interactive prompts.\n"
                      "If you piped this from curl, re-exec with `</dev/tty`.[/red]")
        return 2

    # Ensure chezmoi
    chezmoi_bin = pf.chezmoi or install_chezmoi_interactive(pf.os_name)
    ...
    if not pf.source_exists and use_ssh and not pf.ssh_hint:
        ...
        if key is None:
            use_ssh = False  # fall back to HTTPS
            console.print("[yellow]Falling back to HTTPS clone (no SSH key).[/yellow]")

    # Bundle
    bundle_name = cmd.bundle or ask_bundle()
