...
# info/success/warn/error/skip + colour detection, from scripts/lib/
# log_shared.sh. Sourced (not inlined): this script only ever runs from the
# repo checkout. Two renames came with the move: this file's `ok` is the
# shared `success` (the name `ok` is now the ✔ counter helper), and the
# hand-padded [OK]/[INFO] alignment is gone.
  ...
  if ! $DRY_RUN; then
    ...
    case "$status" in
      ...
      locked)
        ...
        ;;
      unlocked)
        success "Vault already unlocked."
        ;;
    esac
