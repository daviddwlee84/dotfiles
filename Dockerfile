...
ARG CHEZMOI_INSTALL_SUMMARIZE=false
ARG CHEZMOI_INSTALL_AI_DESKTOP_APPS=false
ARG CHEZMOI_AGENT_SOUNDS=notify
ARG CHEZMOI_INSTALL_PERSONAL_TOOLS=false
ARG CHEZMOI_INSTALL_PYTHON_UV_TOOLS=false
ARG CHEZMOI_INSTALL_JS_CLI_TOOLS=false
ARG CHEZMOI_INSTALL_DOTNET_TOOLS=false
ARG CHEZMOI_INSTALL_TERMINAL_BROWSER=false
ARG CHEZMOI_INSTALL_PLAYWRIGHT_CLI=false
ARG CHEZMOI_PRELOAD_PLAYWRIGHT_CHROMIUM=false
...
ARG CHEZMOI_INSTALL_WAKE_ON_LAN=false
...
ARG CHEZMOI_INSTALL_CALIBRE=false
ARG CHEZMOI_INSTALL_MOLE=false
...
ARG CHEZMOI_INSTALL_TAILSCALE=false
...
RUN export PATH="$HOME/.local/bin:$PATH" && \
    ...
    --promptBool "Install coding agents (Claude Code, Pi, Oh My Pi, pia presets, OpenCode, Cursor, Copilot, Gemini, etc.)=${CHEZMOI_INSTALL_CODING_AGENTS}" \
    --promptBool "Install local LLM tools (Ollama, LiteLLM, llmfit, models)=${CHEZMOI_INSTALL_LLM_TOOLS}" \
    --promptBool "Install summarize (YouTube/web/PDF summarizer CLI)=${CHEZMOI_INSTALL_SUMMARIZE}" \
    ...
    --promptChoice "Agent completion feedback (none|notify|peon|both)=${CHEZMOI_AGENT_SOUNDS}" \
    --promptBool "Install personal CLI suite (dev, translate, exp and lazy tools)=${CHEZMOI_INSTALL_PERSONAL_TOOLS}" \
    ...
    --promptBool "Install terminal-browser (includes Electron/Chromium)=${CHEZMOI_INSTALL_TERMINAL_BROWSER}" \
    --promptBool "Install Playwright CLI (browser engines downloaded separately)=${CHEZMOI_INSTALL_PLAYWRIGHT_CLI}" \
    --promptBool "Preload Chromium for Playwright CLI during chezmoi apply=${CHEZMOI_PRELOAD_PLAYWRIGHT_CHROMIUM}" \
    ...
    --promptBool "Arm Wake-on-LAN on wired NIC(s) so this box can be powered on remotely by a magic packet (persisted via a systemd unit)=${CHEZMOI_INSTALL_WAKE_ON_LAN}" \
    ...
    --promptBool "Install calibre (e-book manager; enables .mobi/.azw/.azw3 metadata previews in yazi)=${CHEZMOI_INSTALL_CALIBRE}" \
    --promptBool "Install mole (macOS cleanup, disk analyze, dev build-artifact purge)=${CHEZMOI_INSTALL_MOLE}" \
    ...
    --promptBool "Install Tailscale (mesh VPN; Linux apt/yum repo — macOS uses the cask)=${CHEZMOI_INSTALL_TAILSCALE}" \
