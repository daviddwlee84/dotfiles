...
- **Mihomo clients**: [lazyclash](https://github.com/daviddwlee84/lazyclash), a CLI/TUI for local/SSH cores, node/group editing and sharing, proxy environments, VPN diagnosis and explicit native/Docker client setup. macOS/Linux source installs use the Go toolchain under `installExtraRuntimes`, land in `~/.local/bin`, and include generated zsh/bash completions. v0.1.6 shell helpers use the selected data proxy, own per-shell SSH forwards and restore previous env; `LAZYCLASH_PROXY_SHELL=0` retains legacy helpers. Dotfiles apply installs only the CLI; `lazyclash setup` is the separate reviewed core installation workflow.
