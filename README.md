...
- **Mihomo controller**: [lazyclash](https://github.com/daviddwlee84/lazyclash), a CLI/TUI for existing local or SSH-accessible cores. macOS/Linux source installs use the Go toolchain under `installExtraRuntimes`, land in `~/.local/bin`, and include generated zsh/bash completions. Ctrl+S saves local settings directly; password-only SSH uses native authentication and reuses configured persistent sessions. This does not install a core or register controller credentials.
