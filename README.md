...
- **Mihomo controller**: [lazyclash](https://github.com/daviddwlee84/lazyclash), a CLI/TUI for existing local or SSH-accessible cores. macOS/Linux source installs use the Go toolchain under `installExtraRuntimes`, land in `~/.local/bin`, and include generated zsh/bash completions. The pinned v0.1.4 adds Ctrl+S to save settings directly from any form field. This does not install a core or register controller credentials.
