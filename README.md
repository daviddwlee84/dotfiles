...
- **Personal tools**: One `installPersonalTools` switch for `dev`, `translate`, `exp`, `lazychezmoi`, `lazyclash`, `lazymlflow`, and `lazypueue`. Installation is explicit during apply, never triggered by shell startup or first use. See [installation, ownership and migration](docs/tools/personal-tools.md).
