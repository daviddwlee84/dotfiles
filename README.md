...
4. Runs brew bundle (if `installBrewApps` is enabled, or on macOS if `installAiDesktopApps` / `installGamingApps` / `installMole` is enabled)
...
| `installMole` | false | [mole](docs/tools/mole.md) (`mo`) — macOS cleanup / disk analyze, plus `mo purge` for dev build artifacts (`node_modules`, `target`, `.venv`). **macOS only** (upstream refuses non-darwin), Homebrew formula; `personal-mac` / `work-mac` bundles turn it on. Seeds `~/.config/mole/{whitelist,purge_paths}` — read the whitelist replacement-semantics warning in the doc before editing them. |
| `installDotnetTools` | false | .NET 10 SDK via mise (isolated layout) + verified dotnet global tools ([azure-cost-cli](docs/tools/dotnet-tools.md) for Azure cost analysis) |
| `installPersonalTools` | true | Personal CLI suite: dev, translate, exp and ten lazy tools. Full bundles on; cloud-vm/minimal off. All thirteen tools use Homebrew on macOS / verified binary releases on Linux; no Go SDK is needed. Existing hosts without this key retain their legacy selection until explicitly reconfigured. |
...
- `~/.ssh/config` and `~/.ssh/config.d/{git,common}/` — create-only grouped seeds on new machines, with explicit Include order and four-space indentation. Existing layouts are preserved; optional `dev ssh format` / `organize` provide reviewed cleanup ([guide](docs/tutorials/setup_ssh_key_on_remote.md#grouped-ssh-configuration-and-dev-integration)).

- `~/.config/shell/96_ssh_setup.sh` - Shared bash/zsh `ssh-setup-remote` wizard; gum key selection preserves paths with spaces and stops on picker errors or cancellation ([guide](docs/tutorials/setup_ssh_key_on_remote.md#automation-ssh-setup-remote))
...
- `~/.config/nvim/` - Neovim (LazyVim) configuration; Markdown uses project-local Prettier with a Mason-installed fallback ([formatting](dot_config/nvim/README.md)); Codex `Ctrl+G` prompt drafts use quick-edit without Markdown lint or format-on-save ([editor guide](docs/tools/editor.md)); native `unnamedplus` clipboard locally, with copy-only OSC 52 under SSH/Herdr/Zellij so yanks reach the attached client's clipboard without making normal `p` wait on unsupported clipboard reads (pairs with tmux `set-clipboard on`, see [docs](docs/tools/clipboard.md))
...
- `~/.config/herdr/config.toml` + `~/.dotfiles/bin/herdr-grep` + `~/.agents/skills/herdr/SKILL.md` - Herdr config, cross-pane content search, and its official global agent skill (exported from `herdr --skill`, so it always matches the installed binary). Herdr is a Rust terminal multiplexer + AI-agent orchestrator and a **trial tool that coexists with tmux**, not a replacement. Catppuccin theme, tmux-like keybindings, `prefix+d` dev dashboard, `prefix+G` / `Alt+g` lazygit temporary pane, `prefix+Y` Yazi popup in the focused pane cwd, `prefix+t` interactive translator popup, direct runtime-config edit/validate/reload via `prefix+Alt+e`, low-frequency copy helpers under `prefix+y` Quick Actions, herdr-plus Projects layouts, native agent-state indicators, dual-backend `agent-wakeup`/`agent-warmup`, and regex/fixed-string search across current, named, or all running session scrollback—with `herdr-grep --pick` / `prefix+Alt+F` for fzf selection and exact pane jump/attach. Behavior model and documented gaps are in [docs/tools/herdr.md](docs/tools/herdr.md).
  ...
  - The managed UI uses fixed Catppuccin Mocha, distinct status symbols, `machine · agent` on the second agent row, and the stripped task title on the third; machine labels disappear when only Local is present. `Alt+g` and `prefix+G` both open LazyGit in a temporary pane.
...
- **Personal tools**: One `installPersonalTools` switch for `dev`, `translate`, `exp`, `lazychezmoi`, `lazyclash`, `lazymlflow`, `lazypueue`, `lazyansible`, `lazycrontab`, `lazyfind`, `lazypkg`, `lazymermaid`, and `lazyset`. macOS uses the personal tap; Linux uses checksum-verified binaries. Installation is explicit during apply, never triggered by shell startup or first use. New-install baselines include slim source/module packages; apply remains install-only. See [installation, ownership and migration](docs/tools/personal-tools.md).
- **Mihomo clients**: [lazyclash](https://github.com/daviddwlee84/lazyclash), a CLI/TUI for local/SSH cores, node/group editing and sharing, proxy environments, VPN diagnosis and explicit native/Docker client setup. The Personal tools suite installs macOS CLIs through Homebrew and Linux CLIs through verified GitHub releases; legacy hosts retain their existing source-install selection. Completions are generated for zsh/bash. v0.1.6 shell helpers use the selected data proxy, own per-shell SSH forwards and restore previous env; `LAZYCLASH_PROXY_SHELL=0` retains legacy helpers. Dotfiles apply installs only the CLI; `lazyclash setup` is the separate reviewed core installation workflow.
...
just upgrade-go           # non-personal Go tools (Linux gopls)
just upgrade-personal     # selected, installed personal CLIs; preserve the current package owner
