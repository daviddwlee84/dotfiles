...
- `~/.config/herdr/config.toml` + `~/.dotfiles/bin/herdr-grep` - Herdr config and cross-pane content search (Rust terminal multiplexer + AI-agent orchestrator; **trial tool that coexists with tmux**, not a replacement). Catppuccin theme, tmux-like keybindings, `tv`/lazygit command-panes, herdr-plus Projects layouts, native agent-state indicators, and regex/fixed-string search across current, named, or all running session scrollback—with `herdr-grep --pick` / `prefix+Alt+F` for fzf selection and exact pane jump/attach. Managed via a `modify_` tomlkit overlay (enforces our tables every apply, preserves herdr's runtime writeback) plus a standalone CLI. Behavior model (two cwd fields, OSC7 tracking, nesting inside tmux), search semantics, feasibility matrix, and documented gaps (seamless nvim nav, OSC133 copy-mode) in [docs/tools/herdr.md](docs/tools/herdr.md)
...
- **Python tools (via uv)**: thefuck, apprise, sqlit-tui, dotenv, git-filter-repo, mlflow, tmuxp, trafilatura, marimo, jupyter (lab + classic notebook, with marimo-jupyter-extension), [markitdown](docs/tools/office-viewers.md) (Office→Markdown)
