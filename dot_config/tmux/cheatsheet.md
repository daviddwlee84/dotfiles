...
> **Prefix** = `Ctrl + b` · Press inside tmux: `prefix + Space` (or `prefix + e`) for menu, `prefix + ?` for fuzzy key search.

## Most-Used (memorize these first)

| Key | Action |
|-----|--------|
| `prefix + Space` or `prefix + e` | Popup menu (script-driven, height-aware, with submenus) |
...
## Popup Menu Structure (`prefix + Space` / `prefix + e`)
