# 38_workmux.zsh - Workmux (`workmux` / `wm`) tmux + git-worktree orchestrator
...
#
# Why a separate file from 37_worktrunk.zsh: the two tools coexist (worktrunk
# = `wt` for git-worktree CLI / aliases, workmux = `wm` for status icons +
# dashboard + sidebar). Separate files keep the rationale + helpers per-tool
# and let users disable one independently.
...
function wmclear() {
    if [[ "$1" == "--all" ]]; then
        ...
        local -a windows
        windows=("${(@f)$(tmux list-windows -t "$sess" -F '#{window_index}' 2>/dev/null)}")
        local w
        for w in "${windows[@]}"; do
