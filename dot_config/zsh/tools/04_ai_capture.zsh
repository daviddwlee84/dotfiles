...
#
# Env vars (set in ~/.shellrc.adhoc or ~/.config/zsh/secrets.zsh to override
# — that file always wins against the SSOT's `: "${VAR:=…}"` defaults):
#
...
_aiagent_autodetect() {
  local cand
  # Word-split $AICAP_AGENT_PRIORITY (the `${=…}` is zsh-specific; with
  # bash the var is naturally word-split by `for ... in $var`).
  for cand in ${=AICAP_AGENT_PRIORITY:-opencode claude codex cursor-agent}; do
