...
| `go` | 依 [`go_tools/defaults/main.yml`](../../dot_ansible/roles/go_tools/defaults/main.yml) 各工具的 `platforms` 比對 `uname -s`，再執行 `go install <pkg>@latest` 到 `~/.local/bin`。Darwin 選 lazyclash；Linux 選 lazyclash、translate、dev、gopls。後三者在 macOS 仍由 Homebrew 管理（`just upgrade-brew`）。Go 從 mise shim 或 PATH 取得；缺少時跳過，不自行安裝 Go。 |
