# `scripts/fleet/apply.py` vs. 2018 年代的 Fabric (`fabfile.py`)
...
對本倉庫的 [`scripts/fleet/apply.py`](../../scripts/fleet/apply.py) 與作者較早期的
...
| 關注點 | RaspPi-Cluster `fabfile.py` (Fabric 1.x) | `scripts/fleet/apply.py` (asyncssh) |
|---|---|---|
| 主機清冊 (inventory) | `env.hosts` + `env.passwords` 直接寫死於檔案頂端 | `~/.config/fleet/machines.toml`（`load_hosts`，[scripts/fleet/apply.py:101](../../scripts/fleet/apply.py#L101)） |
| 平行分派 | `@parallel` decorator（執行緒池 (thread pool) 包住 paramiko） | `asyncio.TaskGroup` + 信號量 (semaphore)（單執行緒協程 (coroutine)） |
| Sudo 密碼注入 | `env.passwords[host] = pw`；Fabric 預期並餵入提示 | 推送至遠端 `~/.cache/chezmoi-fleet/sudo.pass`（0600）+ `CHEZMOI_SUDO_PASSWORD_FILE`（[scripts/fleet/apply.py:240](../../scripts/fleet/apply.py#L240)） |
| 遠端命令執行 | `sudo()` / `run()` | `conn.create_process(cmd)`（[scripts/fleet/apply.py:807](../../scripts/fleet/apply.py#L807)） |
| ssh_config 別名 (alias) 支援 | 原生（Fabric 透過 OpenSSH 的 shell 子程序） | asyncssh 為該別名讀取 `~/.ssh/config`（[scripts/fleet/apply.py:550](../../scripts/fleet/apply.py#L550)） |
...
- **scripts/fleet/apply.py**：單執行緒 `asyncio` + `asyncssh`。可擴展性更佳；信號量 (`--max-parallel`) 在沒有執行緒開銷的情況下界定並行數。

### 2. Ctrl+C / 通道斷線時的 process tree 清理

fabfile 完全沒做——在協調器上按 `Ctrl+C` 會在每台 Pi 上留下孤兒 (orphan) `apt-get` / `pip install` 程序。

`scripts/fleet/apply.py` 把清理疊了三層
...
（[scripts/fleet/apply.py:897](../../scripts/fleet/apply.py#L897)）。Fabric 只會印出交錯的 stdout。
...
- **scripts/fleet/apply.py**：密碼**只寫一次**到遠端的 0600 檔案，然後由
...
`scripts/fleet/apply.py` 支援四種（[scripts/fleet/apply.py:59](../../scripts/fleet/apply.py#L59)）：
...
（[scripts/fleet/apply.py:293–299](../../scripts/fleet/apply.py#L293)）。可防範：
...
- **建置成本**：`pip install fabric && fab task`——不用 asyncio、不用 Rich、不用 TOML schema。`scripts/fleet/apply.py` 是一份 1200 行、能 uv self-installing 的腳本。
- **命令式 DSL**：Fabric 讓你直接內嵌寫 `sudo("apt install foo"); put("local.conf", "/etc/foo.conf"); run("systemctl restart foo")`。`scripts/fleet/apply.py` 只執行**一個**命令（`chezmoi update`/`apply`/`diff`），因為每台主機的具體邏輯是委派給 chezmoi + ansible 的。如果你想要臨時即興地 ssh fan-out，Fabric（或它的現代後繼者 [Fabric 2.x](https://www.fabfile.org/) / `pyinfra`）仍然是較合適的選擇。
- **抽象層較低 = 學習成本較低**：`fab -H host1,host2 -- uname -a` 可以開箱即用。`scripts/fleet/apply.py` 需要一份 TOML 檔加上 chezmoi 設定。

## TL;DR

| | Fabric 年代 (2018) | `scripts/fleet/apply.py`（現在） |
|---|---|---|
