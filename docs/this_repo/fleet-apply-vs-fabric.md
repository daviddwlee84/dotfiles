...
A side-by-side comparison between this repo's [`scripts/fleet/apply.py`](../../scripts/fleet/apply.py)
...
| Concern | RaspPi-Cluster `fabfile.py` (Fabric 1.x) | `scripts/fleet/apply.py` (asyncssh) |
|---|---|---|
| Host inventory | `env.hosts` + `env.passwords` hard-coded at top of file | `~/.config/fleet/machines.toml` (`load_hosts`, [scripts/fleet/apply.py:101](../../scripts/fleet/apply.py#L101)) |
| Parallel fan-out | `@parallel` decorator (thread pool over paramiko) | `asyncio.TaskGroup` + semaphore (single-thread coroutines) |
| Sudo password injection | `env.passwords[host] = pw`; Fabric expects + feeds prompt | Pushed to remote `~/.cache/chezmoi-fleet/sudo.pass` (0600) + `CHEZMOI_SUDO_PASSWORD_FILE` ([scripts/fleet/apply.py:240](../../scripts/fleet/apply.py#L240)) |
| Remote command exec | `sudo()` / `run()` | `conn.create_process(cmd)` ([scripts/fleet/apply.py:807](../../scripts/fleet/apply.py#L807)) |
| ssh_config alias support | Native (Fabric shells out to OpenSSH) | asyncssh reads `~/.ssh/config` for the alias ([scripts/fleet/apply.py:550](../../scripts/fleet/apply.py#L550)) |
...
- **scripts/fleet/apply.py**: single-thread `asyncio` + `asyncssh`. Scales further; semaphore (`--max-parallel`) bounds concurrency without thread overhead.
...
([scripts/fleet/apply.py:430–486](../../scripts/fleet/apply.py#L430)):
...
progresses ([scripts/fleet/apply.py:897](../../scripts/fleet/apply.py#L897)). Fabric
just printed interleaved stdout.

### 5. Drift classification (chezmoi-specific)

`_classify_drift()` ([scripts/fleet/apply.py:520](../../scripts/fleet/apply.py#L520))
...
([`run_one_local`, scripts/fleet/apply.py:597](../../scripts/fleet/apply.py#L597)),
...
([scripts/fleet/apply.py:293–299](../../scripts/fleet/apply.py#L293)). Defends against:
...
- **Setup cost**: `pip install fabric && fab task` — no asyncio, no Rich, no TOML schema. `scripts/fleet/apply.py` is a 1200-line uv self-installing script.
- **Imperative DSL**: Fabric let you write `sudo("apt install foo"); put("local.conf", "/etc/foo.conf"); run("systemctl restart foo")` inline. `scripts/fleet/apply.py` only runs **one** command (`chezmoi update`/`apply`/`diff`) because the per-host logic is delegated to chezmoi + ansible. If you want ad-hoc imperative ssh-fanout, Fabric (or its modern successor [Fabric 2.x](https://www.fabfile.org/) / `pyinfra`) is still the better fit.
- **Lower abstraction = less to learn**: `fab -H host1,host2 -- uname -a` works out of the box. `scripts/fleet/apply.py` needs a TOML file and a chezmoi setup.
