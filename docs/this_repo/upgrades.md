...
| `go` | Selects entries for `uname -s` using their `platforms` in [`go_tools/defaults/main.yml`](../../dot_ansible/roles/go_tools/defaults/main.yml), then runs `go install <pkg>@latest` into `~/.local/bin`. Darwin selects lazyclash; Linux selects lazyclash, translate, dev and gopls. The latter three remain Homebrew-managed on macOS (`just upgrade-brew`). Resolves Go via the mise shim or PATH and skips when absent; it does not bootstrap Go. |
