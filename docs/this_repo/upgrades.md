...
| `go` | Upgrades non-personal Go manifest entries (Linux gopls) with the existing Go toolchain; no SDK bootstrap. |
| `personal` | `just upgrade-personal`: selects the suite policy, then upgrades only installed, identified owners. Brew stays Brew, verified Linux releases use checksums, Legacy Go stays source. New lazyansible/crontab/find/pkg/mermaid/set installs use the same Brew/release owners. Missing/disabled/unknown tools are preserved. Successful changes refresh their Bash/Zsh completion. |
...
| `agents` | Updates **tools already present** only. Pi invokes exact `~/.local/bin/pi update --self` with an npm reinstall into its stable prefix as fallback; OMP invokes exact `~/.local/bin/omp update` with `omp.sh/install --binary` fallback. PATH shadows are ignored. Claude Code, OpenCode, Cursor CLI, Ollama (Linux), llmfit (Linux), and RTK retain their guarded self-update/installer paths. Bootstrap list mirrors [`coding_agents`](../../dot_ansible/roles/coding_agents/tasks/main.yml). |
...
| `herdr` | **Only from outside herdr, and only for a self-managed install.** `herdr update --handoff` — the live path that replaces the server without killing pane processes, so coding agents running in panes survive. `SKIPPED` when the binary came from brew/mise/Nix (upstream *disables* `herdr update` there; this repo installs the GitHub-release binary on **both** platforms precisely so the handoff stays available — the message tells you to re-run `chezmoi apply` to migrate) and `SKIPPED` with instructions when `HERDR_ENV`/`HERDR_PANE_ID` show you are inside a pane. After a successful update, the category refreshes `~/.agents/skills/herdr/SKILL.md` from that exact binary's `herdr --skill` output, then reports stale agent integrations without auto-reinstalling them. See [`docs/tools/herdr.md`](../tools/herdr.md) and [`pitfalls/herdr-update-handoff-refuses-inside-pane.md`](https://github.com/daviddwlee84/dotfiles/blob/main/pitfalls/herdr-update-handoff-refuses-inside-pane.md). |
...
Reading the enabled elements correctly is its own topic — `Tokens` is a cumulative sum rather than a water level and excludes subagents, `Cost` is sourced differently and includes them, and the cache countdown is a snapshot that freezes when idle. In v0.8.0, lines wrap between segments when terminal width is known; a single overlong segment is still truncated. The managed order puts cost and the Claude Code version before secondary labels. See [tools/claude-hud.md](../tools/claude-hud.md), including the plan/auth, provider, advisor, and output-style options and recovery from a cached failed version probe.
