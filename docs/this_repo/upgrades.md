...
| `personal` | `just upgrade-personal`: selects the suite policy, then upgrades only installed, identified owners. Brew stays Brew, verified Linux releases use checksums, legacy Go stays source. Missing/disabled/unknown tools are preserved. Successful changes refresh their Bash/Zsh completion. |
