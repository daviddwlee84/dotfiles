...
Reading the enabled elements correctly is its own topic — `Tokens` is a cumulative sum rather than a water level and excludes subagents, `Cost` is sourced differently and includes them, the cache countdown is a snapshot that freezes when idle, and the first line truncates from a fixed segment order that puts `cost` 8th of 10. All of that lives in [tools/claude-hud.md](../tools/claude-hud.md).
