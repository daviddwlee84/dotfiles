...
| `herdr` | **Linux only**, and **only from outside herdr.** `herdr update --handoff` — the live path that replaces the server without killing pane processes, so coding agents running in panes survive. `SKIPPED` on Darwin (homebrew-core owns the binary and upstream *disables* `herdr update` on brew/mise/Nix installs) and `SKIPPED` with instructions when `HERDR_ENV`/`HERDR_PANE_ID` show you are inside a pane — the handoff replaces the server that owns your shell, so herdr refuses. That skip is deliberate: without it, `just upgrade-all` from a herdr pane would fail here every time. Afterwards the category reports any `herdr integration status` entries that went stale but does **not** auto-reinstall them (they land in `~/.claude/hooks/`, `~/.codex/`, `~/.cursor/`, `~/.config/opencode/plugins/` — outside chezmoi's tree). See [`docs/tools/herdr.md`](../tools/herdr.md) and [`pitfalls/herdr-update-handoff-refuses-inside-pane.md`](https://github.com/daviddwlee84/dotfiles/blob/main/pitfalls/herdr-update-handoff-refuses-inside-pane.md). |
...
flowchart LR
    ...
    herdr["herdr<br/>(Linux, update --handoff)"] --> agents
