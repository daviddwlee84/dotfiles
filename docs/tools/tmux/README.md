...
- native popup menu on `prefix + Space`; script-driven (`~/.config/tmux/menu.sh`) with submenus and height-aware tier trimming — see [keybindings.md](./keybindings.md#popup-menu-prefix--space)
...
Shell-level equivalents live in [`03_tmux_capture.zsh`](../../../dot_config/zsh/tools/03_tmux_capture.zsh) — runnable from the prompt, pipe-friendly (`cpout | grep ERROR`), and copy to clipboard as a side effect. Each takes an optional positional `N` for "Nth-latest command" lookback (default 1):

| Command | Equivalent to | What it copies |
|---|---|---|
| `cpout [N]` | `prefix + M-y` (N=1) | Nth-latest command's output only |
| `cpcmd [N]` | `prefix + M-i` (N=1) | Nth-latest command's input line |
| `cpblock [N]` | — | Nth-latest command's full block (prompt + input + output) |
