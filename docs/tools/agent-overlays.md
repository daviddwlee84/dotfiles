...
3. **`SessionEnd` artifact observer** is synchronous and fast: it calls `dev artifact observe-session-end` only to mark an already armed `DEV_AGENT_RUN_ID`. It never stages/commits during Claude teardown; the outer SpecStory wrapper owns post-writer finalization. Peon remains an independent async hook in the same event array.
