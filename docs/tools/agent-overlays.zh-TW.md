...
3. **`SessionEnd` artifact observer** 是同步且快速的：它只呼叫 `dev artifact observe-session-end`標記已armed的 `DEV_AGENT_RUN_ID`，不在Claude teardown內stage/commit。Post-writer finalization由外層SpecStory wrapper負責；peon仍是同一event array內獨立的async hook。
