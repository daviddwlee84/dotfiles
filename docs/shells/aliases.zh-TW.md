...
> 用 **GitHub Copilot** served 的模型驅動 Claude Code 與 Codex，透過只安裝一次、版本釘選的 [copilot-api 維護 fork](https://github.com/caozhiyuan/copilot-api)。Claude Code 優先可選的 Claude model，否則依 live catalog 建立 main/Terra/Luna 角色 profile；Codex 優先 native Responses OpenAI model。完整指南、**ToS / rate-limit 風險**與設定分層：[docs/tools/copilot-claude-proxy.md](../tools/copilot-claude-proxy.md)。首次需執行 `copilot-proxy auth`；launcher 不碰 committed project settings 或 chezmoi-managed user settings。

| Command | Type | Source File | Description |
|---------|------|-------------|-------------|
| `copilot-proxy [start\|stop\|status\|stats\|events\|limiter\|logs\|quota\|bench\|update\|rollback\|doctor\|…]` | function | `dot_config/shell/43_copilot_proxy.sh` | 管理並量測釘選 proxy。Exact update 直接安裝已驗證的 archive；離線 `rollback` 還原套件與 transport 設定，保留目前憑證與用量。`:4142` shim 以不含內容的 metrics 追蹤完成、draining 及 unknown 工作，配對 restart 只在兩個 process 都退出後清除保留的 admission。自適應並行仍為 4→8；`limiter status/set/reset` 即時調整，`COPILOT_SHIM_MIN/MAX` 保留跨 restart 預設。支援 `logs [-f]`、`stats/events --json` 與有上限但消耗 quota 的 `bench`。completion：`61_copilot_proxy_completion.{zsh,bash}`。 |
| `claude-copilot [--fast] [--no-specstory] [args...]` | function | `dot_config/shell/43_copilot_proxy.sh` | 一次性走 proxy 的 Claude Code session，**零檔案寫入**：自動啟動 proxy，per-process 注入完整 role-aware `ANTHROPIC_*` env。既有 `copilot-here` local pin 會蓋過這層。specstory passthrough 會保留設定的 `claude_cmd`；下次直接跑 `claude` 即還原。 |
| `codex-copilot [--no-specstory] [args...]` / `codex-copilot-once` | function | `dot_config/shell/43_copilot_proxy.sh` | 零持久化的 Codex launcher，走本機 Responses gateway。自動啟動 proxy/shim；明確 `-m` 優先，否則即時排序 OpenAI/Codex → Claude → grok → Gemini。注入 live limits 與 provider 專用 `model_catalog_json`：精確 bundled descriptor 保留 reasoning/tools，僅更新 context/max-context。原子 XDG cache 以 Codex version/schema/live-context-map hash 區分；明確 user catalog 會跳過產生，GPT-6 Sol/Luna 缺少精確 descriptor 時要求升級至 Codex 0.156.1+。預設整合 SpecStory 並保留 project/user `codex_cmd`；raw `--no-specstory` 會自行預設 `--ask-for-approval never --sandbox danger-full-access`，使用者明確指定任一軸時各自優先。兩個名稱都不改 Codex config。 |
...
| `copilot-model [<id>\|-l\|-L\|-c\|--auto [--why]\|--why\|--json]` | function | `dot_config/shell/43_copilot_proxy.sh` | 切換 main 並產生完整 Claude Code role profile。`--auto` 會排除 disabled／hidden／embedding／`-fast` entries，依 catalog capability tier 排序，並由 curated allowlist override：Claude > OpenAI > grok > Gemini。選擇不受舊 state／pin 或方案標籤限制。已知 OpenAI main 依 `gpt-6-astra` > `gpt-6-sol` > `gpt-5.6-sol` 排序；`gpt-6-luna` 保持 lightweight，優先用於 Haiku/background。Sol/Luna 使用 live prompt budget，70% compact ratio 僅套用 Astra。`-L` 顯示 tier／price／context／plan metadata；`--why` 不寫入只解釋；`--json` 輸出 raw catalog；`-c` 顯示完整 profile。`[1m]` 與 compact ceiling 都來自 metadata。 |
