...
| `copilot-model [<id>\|-l\|-L\|-c\|--auto [--why]\|--why\|--json]` | function | `dot_config/shell/43_copilot_proxy.sh` | 切換 main 並產生完整 Claude Code role profile。`--auto` 會排除 disabled／hidden／embedding／`-fast` entries，依 catalog capability tier 排序，並由 curated allowlist override：Claude > OpenAI > grok > Gemini。選擇不受舊 state／pin 或方案標籤限制：即使是 fresh shell，可選的 `gpt-6-astra` 仍優先於 Mini；但不會把假想的輕量 `gpt-6-luna` 推到旗艦前。`-L` 顯示 tier／price／context／plan metadata；`--why` 不寫入只解釋；`--json` 輸出 raw catalog；`-c` 顯示完整 profile。`[1m]` 與 compact ceiling 都來自 metadata。 |
