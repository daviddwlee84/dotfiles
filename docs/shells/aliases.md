...
| `codex-copilot [--no-specstory] [args...]` / `codex-copilot-once` | function | `dot_config/shell/43_copilot_proxy.sh` | Zero-persistence Codex launcher on the local Responses gateway. Auto-starts proxy/shim; explicit `-m` wins, otherwise live ranking is OpenAI/Codex → Claude → grok → Gemini. Injects live token limits plus a versioned bundled `model_catalog_json`, isolating it from the provider-global `~/.codex/models_cache.json` that can otherwise lose Sol metadata. Defaults to SpecStory and preserves project/user `codex_cmd`; raw `--no-specstory` independently defaults to `--ask-for-approval never --sandbox danger-full-access`, with each user-specified axis winning independently. Neither name edits Codex config. |
