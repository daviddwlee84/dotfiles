...
> Back Claude Code with models served by a **GitHub Copilot** subscription via the maintained [copilot-api fork](https://github.com/caozhiyuan/copilot-api), installed once at a pinned version. Claude is preferred when entitled; otherwise the helpers build a Sol/Terra/Luna role profile. Full guide + **ToS/rate-limit risks** + settings-layer design: [docs/tools/copilot-claude-proxy.md](../tools/copilot-claude-proxy.md). One-time `copilot-proxy auth` is required. Reversible enable layers keep both committed project settings and chezmoi-managed user settings untouched.

| Command | Type | Source File | Description |
|---------|------|-------------|-------------|
| `copilot-proxy [start\|stop\|restart\|status\|doctor [--live]\|logs [N]\|whoami\|auth]` | function | `dot_config/shell/43_copilot_proxy.sh` | Manage the pinned proxy (`@jeffreycao/copilot-api@2.1.0`). `doctor` distinguishes stale model cache, geo/policy filtering and network failure, then validates the main model plus Fable/Opus/Sonnet/Haiku aliases; no Claude entitlement is a warning when the OpenAI profile works. `--live` costs one quota unit. `auth` explicitly starts GitHub Copilot device login. |
