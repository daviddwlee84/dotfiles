...
| `tv agent-wakeup` | tv channel | `dot_config/television/cable/agent-wakeup.toml` | Dual tmux/Herdr quota dashboard: detects rate/session-limit waits, shows reset time, schedules/cancels identity-safe pueue-backed `continue` wakeups |
...
| `herdr-code` / `hcode` | function / alias | `dot_config/shell/24_herdr.sh` | **herdr** analog of `scode` (needs `herdr` + `jq`). Builds workspace `coding-agent/<repo>`: `editor` tab (nvim 75% \| agent 25%) + `monitor` tab (btop). Refuses outside git repos. Attaches when run outside herdr. Flags: `--on-exit`, `--no-specstory`, `-a/--agent`, `--session NAME` |
| `herdr-here` / `hhere` | function / alias | `dot_config/shell/24_herdr.sh` | **herdr** analog of `shere` (needs `herdr` + `jq`). Lightweight: creates a plain workspace at `$PWD` (or `-p DIR`), focuses + attaches it — **no** git repo and **no** agent layout, just a shell. Bare args / `-c CMD` run in the root pane. Idempotent per dir-basename. `-p DIR` is absolutized in-shell (relative paths OK; a non-directory is a hard error — herdr resolves a relative `--cwd` server-side and would silently open `$HOME`). Flags: `-p/--path`, `-c/--command`, `--no-attach`, `--session NAME` |
| `herdr-root` / `hroot` | function / alias | `dot_config/shell/24_herdr.sh` | **herdr** analog of `sroot`. Like `hhere` but the workspace opens at the current git root (falls back to `$PWD`). Thin wrapper over `hhere`; all flags except `--path` pass through |
| `herdr-mark` / `hmark` | function / alias | `dot_config/shell/24_herdr.sh` | Flag a herdr pane as **review-pending** (⭐) — defaults to the current pane (`$HERDR_PANE_ID`), or pass `hmark w1:p1`. Uses a herdr metadata token (`review`), orthogonal to agent state, so peeking into a done pane (→ idle) does **not** clear it. Also on `prefix+m` (toggle); list flagged panes via `tv herdr-review` / `prefix+i`. Logic in `~/.config/herdr/review-mark.sh` |
| `herdr-unmark` / `hunmark` | function / alias | `dot_config/shell/24_herdr.sh` | Clear a pane's review-pending (⭐) flag (default: current pane) |
...
| `agent-wakeup` | function | `dot_config/shell/62_agent_wakeup.sh` | Dual tmux/Herdr quota dashboard and pueue scheduler; ambient backend by default, `status --backend all` aggregates both |
| `agent-continue-at` | function | `dot_config/shell/62_agent_wakeup.sh` | Shortcut for `agent-wakeup schedule`; `--current` uses `$HERDR_PANE_ID` or `$TMUX_PANE`, with explicit `--backend` available |
| `agent-warmup-update-completion` | function | `dot_config/shell/10_aliases.sh` | Clear and regenerate the Tyro completion cache for `agent-warmup` (zsh + bash caches auto-refresh on executable mtime) |
...
> Requires `dev-cli` (`dev`), `worktrunk` (`wt`) and/or `workmux` (`wm`). `wtcd` also requires `jq` + `fzf`.
>
...
| Command | Type | Source File | Description |
|---------|------|-------------|-------------|
| `dev` | function | `dot_config/shell/39_dev.sh` | Wrapper around the `dev` binary that preserves its TTY while carrying parent-directory changes over a child-only file descriptor; generated by `dev shell-init` for bash/zsh |
...
| `ssh-setup-remote` | function | `dot_config/shell/96_ssh_setup.sh` | Interactive wizard: select/create an SSH key (auto-repairing a `.pub` missing next to a private key), `ssh-copy-id` it to the remote (or an equivalent PowerShell path for a Windows sshd remote, including `administrators_authorized_keys` for admin accounts), then wire it into local `~/.ssh/config` — **edits the existing `Host` block in place** if the alias is already configured (following `Include config.d/*` recursively), else appends a new alias and offers to add a missing `Include`. **Resolves the target's full `ProxyJump` chain first** and repeats the whole flow for every jump host, outermost first — a target reached through a jump host used to end up passwordless only on the last hop. In both bash and zsh, gum selection errors or cancellation stop the current step without taking its default. Prompts use [`gum`](../tools/gum.md) when there is a terminal and gum is installed (arrow keys, a real key picker), falling back to bash `read -e` / zsh `vared` and finally to plain `read`. Env knobs: `SSH_SETUP_ASSUME_YES=1` (take every prompt's default), `SSH_SETUP_KEY=<path>` (skip the key picker), `SSH_SETUP_NO_MUX=1` (disable the per-run `ControlMaster`), `SSH_SETUP_NO_GUM=1` (plain prompts even when gum is present). Needs `python3` for in-place `~/.ssh/config` edits; falls back to append-only otherwise. |
...
| `ytmv <subcommand> [ARGS…]` | CLI (bin) | `dot_dotfiles/bin/executable_ytmv` | Download YouTube music videos for **old MP3 players**: mp3 + sidecar `.lrc` + ID3v2.3/USLT lyrics, or mp4 with soft/burned-in subtitles. Subcommands `get` / `lyrics` / `tag` / `doctor` plus a complete `help [SUBCOMMAND]` setup guide; public downloads use packaged EJS + managed Node before any cookie fallback; compatibility profiles (`safe`, `cjk-big5`, `cjk-gbk`, `ipod`, `modern`) are re-appliable offline via `ytmv tag`. Lyrics from LRCLIB, YouTube captions as fallback. Tab completion `60_ytmv_completion.{zsh,bash}`. See [docs/tools/ytmv.md](../tools/ytmv.md). `ytmv doctor` |
...
> Back Claude Code and Codex with models served by a **GitHub Copilot** subscription via the maintained [copilot-api fork](https://github.com/caozhiyuan/copilot-api), installed once at a pinned version. Claude Code prefers selectable Claude models and otherwise derives the main/Terra/Luna role profile from the live catalog; Codex prefers native Responses OpenAI models. Full guide + **ToS/rate-limit risks** + settings-layer design: [docs/tools/copilot-claude-proxy.md](../tools/copilot-claude-proxy.md). One-time `copilot-proxy auth` is required. Reversible launchers keep committed project settings and chezmoi-managed user settings untouched.

| Command | Type | Source File | Description |
|---------|------|-------------|-------------|
| `copilot-proxy [start\|stop\|status\|stats\|events\|limiter\|logs\|quota\|bench\|update\|rollback\|doctor\|…]` | function | `dot_config/shell/43_copilot_proxy.sh` | Manage and measure the pinned proxy. Exact updates install the verified archive; offline `rollback` restores package/transport settings while preserving current credentials/usage. The `:4142` shim tracks completed, draining and unknown work in content-free metrics; controlled paired restart clears retained admission only after both processes exit. Adaptive concurrency remains 4→8, with `limiter status/set/reset` for live changes and `COPILOT_SHIM_MIN/MAX` for persistent defaults. `logs [-f]`, `stats/events --json` and bounded quota-consuming `bench` remain available. Tab completion: `61_copilot_proxy_completion.{zsh,bash}`. |
| `claude-copilot [--fast] [--no-specstory] [args...]` | function | `dot_config/shell/43_copilot_proxy.sh` | One-off Claude Code session on the proxy: auto-starts it and injects the role-aware `ANTHROPIC_*` env plus a live `max_prompt_tokens`-derived auto-compact window. `--no-specstory` still asserts the same trusted `--dangerously-skip-permissions` posture unless the caller supplies an explicit permission flag. Explicit/Fast models resolve their own ceiling; an unsafe larger sticky-pin ceiling is rejected. Auto-wraps in SpecStory and guards the user-level model key. |
| `claude-copilot-once [--fast] [--no-specstory] [args...]` | function | `dot_config/shell/43_copilot_proxy.sh` | One-shot session via the **`copilot-here` pin**, auto-reverted: checks the proxy, writes the same metadata-derived compact ceiling, runs `claude-copilot`, and unpins on exit/Ctrl-C. Existing pins are drift-checked against the actual launch model, so `--fast` no longer offers to downgrade its own fast pin; wrapper flags must precede Claude args. Needs `jq`. |
| `codex-copilot [--no-specstory] [args...]` / `codex-copilot-once` | function | `dot_config/shell/43_copilot_proxy.sh` | Zero-persistence Codex launcher on the local Responses gateway. Auto-starts proxy/shim; explicit `-m` wins, otherwise live ranking is OpenAI/Codex → Claude → grok → Gemini. Injects live token limits plus a provider-derived `model_catalog_json`: exact bundled descriptors retain reasoning/tools and adopt live context/max-context limits. The atomic XDG cache keys Codex version/schema/live-context-map hash; a caller catalog bypasses generation. GPT-6 Sol/Luna require exact descriptors from Codex 0.156.1+; plain Codex's model cache is untouched. Defaults to SpecStory and preserves project/user `codex_cmd`; raw `--no-specstory` independently defaults to `--ask-for-approval never --sandbox danger-full-access`, with each user-specified axis winning independently. Neither name edits Codex config. |
| `copilot-run <cmd...>` | function | `dot_config/shell/43_copilot_proxy.sh` | Generic building block: auto-start the proxy, then run *any* command with the proxy env injected (e.g. `copilot-run specstory run claude`, `copilot-run claude --resume`) |
| `copilot-here [on\|off\|status]` | function | `dot_config/shell/43_copilot_proxy.sh` | Sticky per-project toggle via gitignored `settings.local.json`. It owns the proxy env and exact Claude auto-compact ceiling; offline same-model refreshes retain the last-known limit with a warning, while offline model changes drop a stale limit. `status` reports model, ceiling, drift and effective launch layers. Needs `jq`. |
| `copilot-model [<id>\|-l\|-L\|-c\|--auto [--why]\|--why\|--json]` | function | `dot_config/shell/43_copilot_proxy.sh` | Switch the main model and derive a full Claude Code profile. `--auto` requires live `/v1/models`, filters non-selectable/embedding/`-fast` entries, and ranks by catalog capability tier with a curated allowlist override: Claude > OpenAI > grok > Gemini. Selection is independent of previous state/pins and billing-plan labels. Known OpenAI main order is `gpt-6-astra` > `gpt-6-sol` > `gpt-5.6-sol`; `gpt-6-luna` stays lightweight and is preferred for Haiku/background. Sol/Luna use their live prompt budget; the 70% compact ratio is Astra-only. `-L` shows tier/price/context/plan metadata, `--why` explains without writing, `--json` emits the raw catalog, and `-c` shows the complete profile. `[1m]` and compact ceilings come from metadata. |
| `copilot-raycast [status\|generate [--dry-run] [--all]\|diff\|probe [MODEL]\|doctor\|edit]` | function | `dot_config/shell/45_copilot_raycast.sh` | Generate Raycast's AI **Custom Providers** file (`~/.config/raycast/ai/providers.yaml`) from the live proxy catalogue, so every Copilot model shows up in Quick AI / AI Chat / AI Commands. Each model id is filtered by a **zero-quota probe** — `POST /chat/completions` with `{"messages":[]}` is rejected before inference, and the error separates usable models (`messages must be non-empty`) from ones `/v1/models` only *claims* to serve (`model_not_supported`, `unsupported_api_for_model`); no field in the static metadata predicts this. `base_url` follows `_copilot_pinned_base` (the throttle shim when it is on), the sweep POSTs to `:4141` direct. Writes are backed up to `~/.local/state/copilot-raycast/backups/`, `yq`-validated and atomically moved — a malformed file makes Raycast **silently** drop *every* custom provider. Other providers in the file are preserved verbatim (needs `yq`). `status`/`doctor` read `raycastAI_modelRouterModelInfo` from Raycast's prefs to show what it actually loaded. Needs `jq` + `curl`; `yq` for validation/preservation. Env: `COPILOT_RAYCAST_{CONFIG,ID,LABEL,SUFFIX,TEMP,JOBS,PROBE_BASE,KEEP}` |
...
| `tsnet-update-completion` | function | `dot_config/shell/10_aliases.sh` | Regenerate lazy-autoload completion for `tsnet` via tyro `--tyro-write-completion` (zsh: `~/.zfunc/_tsnet`; bash: `${XDG_DATA_HOME}/bash-completion/completions/tsnet`); auto-invalidates on binary mtime change in `dot_config/shell/49_tsnet.sh`. See [docs/zsh/zsh-completions.md](../zsh/zsh-completions.md) Section F. |
