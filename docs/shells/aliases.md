...
| Command | Type | Source File | Description |
|---------|------|-------------|-------------|
| `copilot-proxy [start\|stop\|status\|stats\|events\|quota\|bench\|update\|doctor\|…]` | function | `dot_config/shell/43_copilot_proxy.sh` | Manage and measure the pinned proxy. The default `:4142` shim records content-free timings in a separate 90-day SQLite DB and joins them to the fork's token events by trace id; `stats/events --json` also work offline. `bench` is bounded but consumes real quota. `update --check` is read-only; exact-version updates verify SHA-512, stage, smoke-test and rollback. Tab completion: `61_copilot_proxy_completion.{zsh,bash}`. |
