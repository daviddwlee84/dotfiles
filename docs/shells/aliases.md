...
| `copilot-model [<id>\|-l\|-L\|-c\|--auto [--why]\|--why\|--json]` | function | `dot_config/shell/43_copilot_proxy.sh` | Switch the main model and derive a full Claude Code profile. `--auto` requires live `/v1/models`, filters non-selectable/embedding/`-fast` entries, and ranks by catalog capability tier with a curated allowlist override: Claude > OpenAI > grok > Gemini. This selects `gpt-6-astra` as the new flagship without promoting a hypothetical lightweight `gpt-6-luna`. `-L` shows tier/price/context/plan metadata, `--why` explains without writing, `--json` emits the raw catalog, and `-c` shows the complete profile. `[1m]` and compact ceilings come from metadata. |
