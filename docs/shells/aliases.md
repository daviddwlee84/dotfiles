...
| `claude-copilot [--no-specstory] [args...]` | function | `dot_config/shell/43_copilot_proxy.sh` | One-off Claude Code session on the proxy: auto-starts the proxy and injects the complete role-aware `ANTHROPIC_*` env per-process. An active `copilot-here` local pin outranks this env. Auto-wraps in `specstory run claude`; passthrough args preserve the configured `claude_cmd`. Guards `~/.claude/settings.json.model` so `/model` cannot leave a proxy-only global default; unrelated live settings survive. |
...
| `codex-copilot [--no-specstory] [args...]` / `codex-copilot-once` | function | `dot_config/shell/43_copilot_proxy.sh` | Zero-persistence Codex launcher on the local Responses gateway. Auto-starts proxy/shim; explicit `-m` wins, otherwise live ranking is OpenAI/Codex → Claude → Gemini. Injects live token limits plus a versioned bundled `model_catalog_json`, isolating it from the provider-global `~/.codex/models_cache.json` that can otherwise lose Sol metadata. Defaults to SpecStory and preserves project/user `codex_cmd`; neither name edits Codex config. |
...
| `copilot-here [on\|off\|status]` | function | `dot_config/shell/43_copilot_proxy.sh` | Sticky per-project toggle via the **gitignored** `./.claude/settings.local.json` (overrides committed project settings). `on` merges the proxy env; `off` removes it and cleans a proxy-only user-level `model` even when already off; `status` shows pin drift plus a user/project/local/shell launch audit with the effective backend/model and stale running Claude PIDs. Needs `jq` |
| `copilot-model [<id>\|-l\|-c\|--auto]` | function | `dot_config/shell/43_copilot_proxy.sh` | Switch the main model and derive a full Claude Code profile. `--auto` requires live `/v1/models`: Claude first, else Sol > Terra > GPT-5.5 > GPT-5.4 > GPT-5.3 Codex > Luna > mini > Gemini. OpenAI roles map Main/Fable/Opus=Sol, Sonnet=Terra, Haiku/background=Luna; `[1m]` comes from context metadata. Local pins store all roles; global state remains one main-id line. `-c` shows the complete profile. |
