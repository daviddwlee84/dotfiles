...
| `ytmv <subcommand> [ARGS…]` | CLI (bin) | `dot_dotfiles/bin/executable_ytmv` | Download YouTube music videos for **old MP3 players**: mp3 + sidecar `.lrc` + ID3v2.3/USLT lyrics, or mp4 with soft/burned-in subtitles. Subcommands `get` / `lyrics` / `tag` / `doctor`; compatibility profiles (`safe`, `cjk-big5`, `cjk-gbk`, `ipod`, `modern`) are re-appliable offline via `ytmv tag`. Lyrics from LRCLIB, YouTube captions as fallback. Tab completion `60_ytmv_completion.{zsh,bash}`. See [docs/tools/ytmv.md](../tools/ytmv.md). `ytmv doctor` |
