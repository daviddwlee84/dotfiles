...
| Command | Type | Source File | Description |
|---------|------|-------------|-------------|
| `copilot-proxy [start\|stop\|status\|stats\|events\|limiter\|logs\|quota\|bench\|update\|doctor\|…]` | function | `dot_config/shell/43_copilot_proxy.sh` | Manage and measure the pinned proxy. The default `:4142` shim records content-free timings in a separate 90-day SQLite DB and uses an adaptive 4→8 concurrency range; `limiter status/set/reset` changes it live without interrupting streams, while exported `COPILOT_SHIM_MIN/MAX` persist across restart. `logs -f` and `logs shim -f` follow across rotation. `stats/events --json` work offline and separate client cancellations from upstream failures. `doctor` reports that the current fork strips Codex fast `service_tier`. `bench` is bounded but consumes real quota. Tab completion: `61_copilot_proxy_completion.{zsh,bash}`. |
