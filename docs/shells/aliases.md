...
| `appsrc [ARGS…]` | CLI (bin) | `dot_dotfiles/bin/executable_appsrc` | Detect how an installed app / CLI was installed & managed (Homebrew cask/formula, Mac App Store, direct-download dmg/pkg, apt, snap, flatpak, AppImage, cargo/npm/pipx/uv/mise/go/gem, manual). `appsrc which <name>` = live single lookup (a command or a GUI app name); bare `appsrc` / `appsrc scan` = cached batch inventory (`--json` / `--tsv` / `--kind` / `--source` / `--refresh` / `--no-cache`). Picker twin: `tv appsrc`. Tab completion `57_appsrc_completion.{zsh,bash}`. See `docs/tools/appsrc.md`. `appsrc which docker` |
