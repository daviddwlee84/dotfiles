...
| `codex-copilot [--no-specstory] [args...]` / `codex-copilot-once` | function | `dot_config/shell/43_copilot_proxy.sh` | Zero-persistence Codex launcher on the local Responses gateway. Auto-starts proxy/shim; explicit `-m` wins, otherwise live ranking is OpenAI/Codex → Claude → Gemini. Defaults to SpecStory and preserves project/user `codex_cmd`; neither name edits Codex config. |
